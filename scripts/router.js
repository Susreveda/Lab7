 ...
 */
router.setState = function (whichState, whichNum, bool) {
  /**
  ...
  if (whichState.page == "setting") {
  ...
  } else if (whichState.page == "journal") {
    if (!bool) {
      let state = whichState;
      let title = "single entry";
      let path = "/#Entry" + whichNum;
      history.pushState(state, title, path);
    }
    ...
    let top = document.querySelectorAll("h1")[0];
    top.innerHTML = "Entry " + whichState.num;

    let currEntry = document.querySelector("entry-page");
    currEntry.remove();
